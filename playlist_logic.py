...
def classify_song(song: Song, profile: Dict[str, object]) -> str:
    ...
    # Fixed: was checking chill keywords against title; should check genre (consistent with is_hype_keyword)
...
def compute_playlist_stats(playlists: PlaylistMap) -> Dict[str, object]:
    ...
    # Fixed: was len(hype), making hype_ratio always 1.0; use len(all_songs) for correct ratio
    total = len(all_songs)
    hype_ratio = len(hype) / total if total > 0 else 0.0

    avg_energy = 0.0
    if all_songs:
        # Fixed: was summing only hype songs; use all_songs for true average energy
...
def search_songs(
    ...
    for song in songs:
        ...
        # Fixed: was checking if value is substring of query (inverted); check query in value
...
def random_choice_or_none(songs: List[Song]) -> Optional[Song]:
    ...
    # Fixed: random.choice raises IndexError on empty list; guard with None return
